...
class ValidatePaymentForm(FormValidationAction):
    ...
    def validate_expiry(
        ...
        if not expiry:
            ...
            if expiry[2]== "/" and ((int(expiry[0])==0 and int(expiry[1])>=0 and int(expiry[1])<=9) or (int(expiry[0])==1 and int(expiry[1])>=0 and int(expiry[1])<=2))and int(expiry[3])>=2 and int(expiry[4])>=4:
